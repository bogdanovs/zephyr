...
static int crypto_cc23x0_ecb_encrypt(struct cipher_ctx *ctx, struct cipher_pkt *pkt)
{
	...
	while (in_bytes_processed < pkt->in_len) {
				     ...
				     struct cipher_aead_pkt *aead_op, uint8_t *nonce)
{
	...
	for (i = 0 ; i < sizeof(msg_len) ; i++) {
		byte = CCM_CC23_BYTE_GET(i, msg_len);
		memcpy(&b0[AES_BLOCK_SIZE - 1 - i], &byte, 1);
	}
	...
	for (i = 0 ; i < sizeof(ad_len) ; i++) {
		byte = CCM_CC23_BYTE_GET(i, ad_len);
		memcpy(&b1[CCM_CC23_AD_LEN_SIZE - 1 - i], &byte, 1);
	}
...
static int crypto_cc23x0_ctr_drbg(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	...
	/* Write the counter value to the AES engine */
	memcpy(ctr, iv, iv_len);
	AESWriteBUF(ctr);
				       ...
				       enum cipher_op op_type)
{
	...
	if (mode != CRYPTO_CIPHER_MODE_ECB &&
	    ...
	    mode != CRYPTO_CIPHER_MODE_CTR_DRBG) {
	...
	if (op_type == CRYPTO_CIPHER_OP_ENCRYPT) {
		...
		case CRYPTO_CIPHER_MODE_CCM:
			...
			break;
		case CRYPTO_CIPHER_MODE_CTR_DRBG:
			ctx->ops.ctr_drbg_crypt_hndlr = crypto_cc23x0_ctr_drbg;
			break;
		default:
			return -EINVAL;
		}
				      ...
				      struct cipher_ctx *ctx)
{
	...
	if (AESGetStatus() == AES_STA_STATE_BUSY) {
		return -EBUSY;
	}
...
static struct crypto_cc23x0_config crypto_cc23x0_dev_config = {
