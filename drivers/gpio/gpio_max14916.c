...
static int gpio_max14916_manage_diag_callback(const struct device *port,
...
static int gpio_max14916_port_diag_get(const struct device *dev, int chan, uint8_t *data)
{
	...
	{	// Global
		case GPIO_DIAG_ABOVE_VDD:
			ret = data->glob.glob_err.VINT_UV  |
			      data->glob.glob_err.VA_UVLO  |
			      data->glob.glob_err.VDD_BAD  |
			      data->glob.glob_err.VDD_WARN |
...
static int gpio_max14916_diag_fetch(const struct device *dev)
{
	...
	data->glob.interrupt.reg_raw = max149x6_reg_transceive(dev, MAX14916_INT_REG, 0, NULL,
							       MAX149x6_READ);

	data->glob.glob_err.reg_raw = max149x6_reg_transceive(dev, MAX14916_GLOB_ERR_REG, 0, NULL,
							      MAX149x6_READ);

	data->chan.ovr_ld = max149x6_reg_transceive(dev, MAX14916_OVR_LD_REG, 0, NULL,
						    MAX149x6_READ);

	data->chan.curr_lim = max149x6_reg_transceive(dev, MAX14916_CURR_LIM_REG, 0, NULL,
						      MAX149x6_READ);

	data->chan.ow_off = max149x6_reg_transceive(dev, MAX14916_OW_OFF_FLT_REG, 0, NULL,
						    MAX149x6_READ);

	data->chan.ow_on = max149x6_reg_transceive(dev, MAX14916_OW_ON_FLT_REG, 0, NULL,
						   MAX149x6_READ);

	data->chan.sht_vdd = max149x6_reg_transceive(dev, MAX14916_SHT_VDD_FLT_REG, 0, NULL,
						     MAX149x6_READ);
}
...
static int gpio_max14916_chan_diag_get(const struct device *dev, int chan, gpio_pin_t pin)
{
	...
	{	// Add enable bit
		...
		case GPIO_DIAG_WIRE_BREAK:
			ret = ((data->chan.wb_on >> pin) & 0x1) |
			      ((data->chan.wb_off >> pin) & 0x1);
			break;
