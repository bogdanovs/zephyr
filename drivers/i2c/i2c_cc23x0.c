...
static int i2c_cc23x0_transmit(const struct device *dev, const uint8_t *buf,
...
static int i2c_cc23x0_receive(const struct device *dev, uint8_t *buf,
				     uint32_t len,
				     uint16_t addr)
{
	const struct i2c_cc23x0_config *config = dev->config;
...
static int i2c_cc23x0_transfer(const struct device *dev,
				      struct i2c_msg *msgs,
				      uint8_t num_msgs, uint16_t addr)
{
	...
	for (int i = 0; i < num_msgs; i++) {
		...
		if ((msgs[i].flags & I2C_MSG_RW_MASK) == I2C_MSG_WRITE) {
			ret = i2c_cc23x0_transmit(dev, msgs[i].buf,
							 msgs[i].len, addr);
		} else {
			ret = i2c_cc23x0_receive(dev, msgs[i].buf,
							msgs[i].len, addr);
		}
...
static int i2c_cc23x0_configure(const struct device *dev,
				       uint32_t dev_config)
{
	const struct i2c_cc23x0_config *config = dev->config;
	bool fast;
...
static int i2c_cc23x0_init(const struct device *dev)
{
	const struct i2c_cc23x0_config *config = dev->config;
	uint32_t cfg;
	int err;
	...
	IRQ_CONNECT(DT_INST_IRQN(0),
		    ...
		    i2c_cc23x0_isr, DEVICE_DT_INST_GET(0), 0);
...
static const struct i2c_driver_api i2c_cc23x0_driver_api = {
	.configure = i2c_cc23x0_configure,
...
#define CC23X0_I2C(id)									\
	...
	static struct i2c_cc23x0_data i2c_cc23x0_##id##_data = {			\
		...
		.error = I2C_MASTER_ERR_NONE						\
											...
											\
											\
	I2C_DEVICE_DT_INST_DEFINE(id, i2c_cc23x0_init, NULL,				\
				  &i2c_cc23x0_##id##_data, &i2c_cc23x0_##id##_config,	\
				  POST_KERNEL, CONFIG_I2C_INIT_PRIORITY,		\
				  &i2c_cc23x0_driver_api);
